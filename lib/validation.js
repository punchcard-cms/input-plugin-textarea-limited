...
module.exports = function textareaLimitedValidation(input, settings) {
  if (input.target.value && settings.hasOwnProperty('target') && settings.target.hasOwnProperty('minlength') && (settings.target.minlength > input.target.value.length)) {
    return `${input.target.name} requires at least ${settings.target.minlength} characters`;
  }

  if (input.target.value && settings.hasOwnProperty('target') && settings.target.hasOwnProperty('maxlength') && (settings.target.maxlength < input.target.value.length)) {
